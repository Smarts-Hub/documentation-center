...
```
Alternatively you can add `once: true` to execute that event only once. This can be used to avoid interference between modules that uses the same event in a different listener.


## API
To interact with the bot you might want to use its api. Here is a quick summary of how to use it. See [api](/0xbot/module-development/api/) for more information.
